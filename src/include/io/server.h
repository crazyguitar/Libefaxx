  ...
  class Socket : private NoCopy {
   public:
    ...
    void Open(const std::string& addr, int port) {
      ...
      if (gai_err != 0) {
        SPDLOG_ERROR("getaddrinfo({}, {}) fail. error: {}", addr, service, gai_strerror(gai_err));
      ...
      if (fd_ < 0) {
        ...
        addrinfo_ = nullptr;
        SPDLOG_ERROR("Failed to bind to {}:{}", addr, port);
        throw std::runtime_error("Failed to bind socket to " + addr + ":" + std::to_string(port));
      }

      if (listen(fd_, SOMAXCONN) < 0) {
        int saved_errno = errno;
        SPDLOG_ERROR("listen({}, {}) fail. error: {}", fd_, SOMAXCONN, strerror(saved_errno));
        Close();
    ...
    while (!stopped) {
      ...
      if (client_fd >= 0) {
        ...
        if (err == EMFILE || err == ENFILE || err == ENOMEM || err == ENOBUFS) {
          SPDLOG_WARN("accept() resource exhaustion: {}", strerror(err));
          break;
        }
        SPDLOG_WARN("accept() failed: {}", strerror(err));
        break;
      }
    }
  }
